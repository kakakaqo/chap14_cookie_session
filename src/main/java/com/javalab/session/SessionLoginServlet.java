...
//        response.sendRedirect(contextPath + "/session/loginForm.jsp");
   ...
   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      String pwd = "";
      String contextPath = request.getContextPath();
      String url = "/session/loginForm.jsp";   // 로그인 실패시 다시 로그인
      ...
      if (member == null) {
         ...
         HttpSession session = request.getSession();
         // 4.4 세션에 객체 저장
         // (JSP에서 꺼내 쓸때는 ${sessionScope.membe.id})
         // member 라는 이름으로 member 객체 저장
         session.setAttribute("member", member);
         
         //4.5 세션의 유지 시간 설정
         session.setMaxInactiveInterval(3600);    //세션유지시간지정
